...
const nftClassSymbol = `tradeinstruments${Date.now()}`

//const generateKittenURL = () => {
//  return `https://placekitten.com/${200 + Math.floor(Math.random() * 100)}/${200 + Math.floor(Math.random() * 100)}`
//}

const generateTradeInstrumentURL = () => {
  //return `https://placekitten.com/${200 + Math.floor(Math.random() * 100)}/${200 + Math.floor(Math.random() * 100)}`
  //return `./tradedocuments-original/BLG10001.json`
  return `./tradedocuments-original/BL-shoes-1.jpg`
...
const NFT: NextPage = () => {
  ...
  const [tradeInstrumentURI, setTradeInstrumentURI] = useState(generateTradeInstrumentURL())
  ...
  */
  ...
  const changeTradeInstrument = () => {
    setTradeInstrumentURI(generateTradeInstrumentURL())
  }

  const mintTradeInstrument = () => {
    setError('')
    setLoading(true)

    console.log(' nftClassID ' , nftClassID );
      ...
      {transferID == "" && classCreated && (
        <div>
          ...
          <div className="grid grid-flow-col auto-cols-max">
            ...
            <div className="ml-8">
              <img className="rounded-full object-cover h-48 w-48" src={tradeInstrumentURI} alt=""/>
              <div className="py-8">
                <button className="btn btn-primary float-left btn-accent rounded-full" onClick={changeTradeInstrument}>Change
                </button>
                <button className="btn btn-primary float-right rounded-full" onClick={mintTradeInstrument}>Mint</button>
              </div>
            </div>
          </div>
        </div>)}
